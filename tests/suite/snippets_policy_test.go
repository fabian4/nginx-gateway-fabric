...
var _ = Describe("SnippetsPolicy", Ordered, Label("functional", "snippets-policy"), func() {
	...
	When("SnippetsPolicies are applied to the resources", func() {
		...
		Context("nginx directives", func() {
			...
			snippetsPolicyFilePrefix := "/etc/nginx/includes/policy/"
...
func checkForSnippetsPolicyToBeAccepted(snippetsPolicyNsNames types.NamespacedName) error {
	...
	if len(sp.Status.Conditions) == 0 {
		return fmt.Errorf("snippetsPolicy has no conditions")
	}

	condition := sp.Status.Conditions[0]
	if condition.Type != string(ngfAPI.PolicyConditionAccepted) {
	...
	if condition.Reason != string(ngfAPI.PolicyConditionReasonAccepted) {
		wrongReasonErr := fmt.Errorf(
			"expected condition reason to be %s, got %s",
			ngfAPI.PolicyConditionReasonAccepted,
			condition.Reason,
		)
		GinkgoWriter.Printf("ERROR: %v\n", wrongReasonErr)
